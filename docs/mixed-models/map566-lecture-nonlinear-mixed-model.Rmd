...
Here, $\omega^2_{ka}$ and $\omega^2_{ke}$ are the variances of $\log({k_a}_i)$ and $\log({k_e}_i)$ while $\omega^2_{V}$ is the  variance of $V_i$.
...
Consider, for instance a model where $k_a$ is fixed in the population, i.e. $\omega_{k_a}=0$ (and thus ${k_a}_i={k_a}_{\text{pop}}$ for all $i$), and where $\log(V)$ and $\log(k_e)$ are correlated, i.e $\eta_V$ and $\eta_{k_e})$ are correlated:
