...
ggplot(data=U) + geom_line(aes(tau,RSS)) + geom_point(aes(x=tau.est,y=U.min), colour="red", size=2)
```
...
pl1 <- ggplot(data=d1) + geom_line(aes(time, y), color="#6666CC")  +
  ...
  geom_vline(xintercept = (tau.est+0.5), color="red", size=1)
...
Nevertheless, we would come to the same conclusion with $\tau=92$ or $\tau=94$\ldots but probably not with $\tau=90$ or $\tau=110$.
...
* for estimating the expected absolute error  $\esp{|\hat\tau - \tau^\star|}$ by the mean absolute error 
...
Let us display the  ditribution of $\hat{\tau}$ estimated by Monde Carlo simulation with the R function [`cpf`](cpf.R),  with $n=100$ and $\delta=0.5$, 0.75, 1, 2.
...
$$
which means that $\prob{\delta^2|\hat{\tau} - \tau^\star| > A}$ tends to 0 as $A$ goes to infinity. 
...
$$\prob{S_{\rm max} > q_{S_{\rm max},1-\alpha}} = \alpha$$
