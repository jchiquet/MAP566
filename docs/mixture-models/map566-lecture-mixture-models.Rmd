...
Functions from `R`-base and stats (preloaded) are required plus packages from the **tidyverse** for data representation and manipulation. We will also use the package **mixtools**, which implements EM for simple mixture models to check our own implementation.
...
```{r}
clustered_data %>% group_by(cl_hat) %>% summarise(count = n(), means = mean(y), stdev = sd(y))
```
...
```{r, warning=FALSE}
## trick: nlm diverge if two parameters are used for pi, so for ce the summation to 1
...
tau %>% as_tibble() %>% setNames(c("comp.1", "comp.2")) %>% 
...
accuracy <-c(mean(d$z != cl_kmeans)*100, mean(d$z != cl_gmm)*100)
accuracy
```
