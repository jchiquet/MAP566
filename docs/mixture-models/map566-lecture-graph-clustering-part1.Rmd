...
In what follow, a graph $\mathcal{G}=(\mathcal{V},\mathcal{E})$ is a mathematical structure consisting of
  ...
  - a set $\mathcal{V}=\set{1,\dots,n}$ of vertices or **nodes**
  - a set $\mathcal{E}=\set{e_1,\dots,e_p:e_k=(i_k,j_k)\in (\mathcal{V}\times\mathcal{V})}$ of **edges**
...
The idea of AHC applies naturally to network data just as it applies to standard data set:
...
Given a dissimilarity $d_{ij}$ between a pairs $(i,j)$, the standard merging criterion based on $d_{ij}$ are the followings:
...
#### Some AHC algorithm specific to network data

We now consider a couple of methods building on the heuristic of AHC but explicitly dedicated to the task of graph clustering. In aprticular, we look at to dissimilarity that favors groups of nodes (modules) which are densely connected but sparsely  connected to other modules.
...
The following algorithm try to optimise the modularity by successive merges:
...
$$
  \mathbf{x}^\top \mathbf{L} \mathbf{x} = \frac{1}{2} \sum_{i,j} Y_{ij} (x_i - x_j)^2, \quad \forall \mathbf{x}\in\Rset^n .
$$
...
$$
      \mathbf{L}_N = \bD^{-1/2}\mathbf{L}\bD^{-1/2} = \bI - \bD^{-1/2} \bA \bD^{-1/2}.
$$

The **absolute** Laplacian matrix $\mathbf{L}_{abs}$ (see @rohe2011spectral) is defined by 

$$
  \mathbf{L}_{abs} = \bD^{-1/2}\bA\bD^{-1/2} = \bI - \mathbf{L}_N,
$$
