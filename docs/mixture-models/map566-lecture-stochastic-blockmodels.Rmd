...
Also, and since the original motivation of the graph-cut is for two-way partionning, we collapse levels from the vector of party into a simplified left/right view (we keep the analysts into a third speparated group)
...
Based on these two properties, spectral clustering algorithms follow the following genneral principles:
...
1. Computethe laplacian matrix $\mathbf{L}$\;
