...
Recall tha we have  *fixed* nodes $\{1, \dots, n \}$ with **hidden** colors $\mathcal{C}=\{\color{#fab20a}{\bullet},\color{#0000ff}{\bullet},\color{#008000}{\bullet}\}$. 
...
$$\begin{aligned}
\ell_c(\mathbf{Y},\mathbf{Z}; \theta)  & =  p(\mathbf{Y} | \mathbf{Z}; \boldsymbol\pi) p(\mathbf{Z} ; \boldsymbol\alpha)\nonumber = \prod_{i,j} f_{\pi_{Z_i,Z_j}}(Y_{ij}) \times   \prod_{i} \alpha_{Z_i} \nonumber  \\
