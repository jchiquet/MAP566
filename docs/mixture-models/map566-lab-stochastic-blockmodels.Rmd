...
1. Extract the binary tree-tree adjacency matrix and plot it (`plotMyMatrix`)
2. Plot the adjacency matrix. Construct and build an igraph object to plot the network 
3. Implement different variant of spectral clustering and test them on this data. Plot the corresponding reordered adjacency matrix.
4. Adjust a simple binary SBM with `estimateSimpleSBM`. Become familiar with object `simpleSBM`.
5. Select a model via ICL. Explore/check models with similar ICL (using `$setModel()`, `$storedModel`)
...
2. Compare spectral, binary SBM and Poisson SBM with ARI/NID (package `aricode`) and alluvial plots (`plotAlluvial`)
